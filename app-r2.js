...
window.CULINARY_APP_STARTED = true;
...
function renderCard(card) {
  ...
  if (card.image) {
    ...
    $("imageCaption").textContent = card.imageCaption || "";
...
if (cards.length) {
  ...
  $("libraryCount").textContent = "Content database not found";
  $("body").innerHTML =
    '<div class="load-error"><strong>cards-r2.js is missing or did not upload.</strong>' +
    '<p>Upload it to the same level as index.html and app-r2.js.</p></div>';
}

if ("serviceWorker" in navigator) {
  window.addEventListener("load", () => navigator.serviceWorker.register("sw-r2.js").catch(console.error));
}
